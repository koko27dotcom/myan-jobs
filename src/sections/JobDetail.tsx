  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-20 pb-24">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-gradient-to-r from-green-500 to-emerald-600 rounded-2xl p-6 text-white mb-6 shadow-lg relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <button 
              ...
              className="px-6 py-3 bg-white text-green-600 rounded-xl font-semibold hover:shadow-lg transition-all active:scale-95 pointer-events-auto"
            >
              Refer Now
            </button>
          </div>
      ...
      {/* APPLY MODAL - With AnimatePresence */}
      <AnimatePresence>
        {showApply && (
          <motion.div 
            initial={{ opacity: 0 }}
          ...
          >
            ...
            >
              ...
              <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setShowApply(false); alert('Application submitted!'); }}>
                <input required type="text" placeholder="Full Name" className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 outline-none focus:ring-2 focus:ring-blue-200 transition-all" />
                <input required type="email" placeholder="Email" className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 outline-none focus:ring-2 focus:ring-blue-200 transition-all" />
                <input required type="tel" placeholder="Phone" className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 outline-none focus:ring-2 focus:ring-blue-200 transition-all" />
                
                {/* FILE UPLOAD */}
                <div className="border-2 border-dashed border-slate-300 rounded-xl p-4 text-center hover:border-blue-500 hover:bg-blue-50 transition-all cursor-pointer">
                  <input type="file" id="resume" className="hidden" accept=".pdf,.doc,.docx" />
                  <label htmlFor="resume" className="cursor-pointer flex flex-col items-center gap-2 pointer-events-auto">
                    <Upload size={24} className="text-slate-400" />
                    <span className="text-sm text-slate-600">Click to upload Resume/CV (PDF, DOC)</span>
                  </label>
                </div>

                <button type="submit" className="w-full py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl font-semibold hover:shadow-lg transition-all active:scale-95">
                  Submit Application
                </button>
              </form>
            </motion.div>
      ...
      {/* REFER MODAL - With AnimatePresence */}
      <AnimatePresence>
        {showRefer && (
          <motion.div 
            initial={{ opacity: 0 }}
